...
Size SpriteController::GetContentSize(Node* node) {
    if (node) {
        auto originalSize = node->getContentSize();
        auto scaledSize = Size(originalSize.width * node->getScaleX(),
            originalSize.height * node->getScaleY());
        return scaledSize;
    }
    return Size::ZERO;
}
