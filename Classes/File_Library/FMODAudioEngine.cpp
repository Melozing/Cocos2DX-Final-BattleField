...
bool FMODAudioEngine::lazyInit()
{
    ...
    // Add DSP to the master channel group
...
void FMODAudioEngine::getSpectrum(float* spectrumArray, int numValues)
{
    if (_dsp) {
        ...
        _dsp->getParameterData(FMOD_DSP_FFT_SPECTRUMDATA, (void**)&fftParameter, nullptr, nullptr, 0);
        if (fftParameter && fftParameter->length > 0) {
            int numChannels = fftParameter->numchannels;
            for (int i = 0; i < numValues; ++i) {
                spectrumArray[i] = 0.0f;
                for (int j = 0; j < numChannels; ++j) {
                    spectrumArray[i] += fftParameter->spectrum[j][i];
                }
                spectrumArray[i] /= numChannels;
            }
        }
    }
}
...
std::vector<float> FMODAudioEngine::getFrequencyData() {
    std::vector<float> spectrum(512, 0.0f); // Initialize with 512 zeros
    if (_dsp) {
        FMOD_DSP_PARAMETER_FFT* fftParameter = nullptr;
        _dsp->getParameterData(FMOD_DSP_FFT_SPECTRUMDATA, (void**)&fftParameter, nullptr, nullptr, 0);
        if (fftParameter && fftParameter->length > 0) {
            ...
            for (int i = 0; i < spectrum.size(); ++i) {
                spectrum[i] = 0.0f;
                for (int j = 0; j < numChannels; ++j) {
                    spectrum[i] += fftParameter->spectrum[j][i];
                }
                spectrum[i] /= numChannels;
            }
        }
    }
    return spectrum;
